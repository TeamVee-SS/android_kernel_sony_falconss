...
int EnterISPMode(struct i2c_client *client, uint8_t *isp_cmd)
{
	...
	if (len != sizeof(buff)) {
		pr_err("[ELAN] %s: Fail! len = [%d]\r\n", __func__, len);
		return -1;
	}
	pr_info("[ELAN] %s: IAPMode write data successfully! cmd = [%2x, %2x, "
		"%2x, %2x]\n",
		__func__, isp_cmd[0], isp_cmd[1], isp_cmd[2], isp_cmd[3]);
	return 0;
}
...
int GetAckData(struct i2c_client *client)
{
	...
	if (len != sizeof(buff)) {
		pr_err("[ELAN] %s: read data error, write 50 times error. "
		       "len = [%d]\r\n",
		       __func__, len);
		return -1;
	}
...
static int elan_ktf2k_ts_setup(struct i2c_client *client)
{
	...
	pr_info("[ELAN] %s: first hellopacket's rc = [%d]\n", __func__, rc);
	...
	if (rc != 0x80 && rc != 0x88) {
		...
		if (rc < 0)
			pr_err("[ELAN] %s: fw_packet_handler fail, rc = [%d]",
			       __func__, rc);
		...
		if (FW_VERSION == 0x00) {
			rc = 0x80;
			pr_err("[ELAN] %s: Last FW update fail, FW_VERSION = "
			       "[%d]\n",
			       __func__, FW_VERSION);
		}
	}
...
static int elan_ktf2k_ts_rough_calibrate(struct i2c_client *client)
{
	...
	dev_info(&client->dev,
		 "[ELAN] %s: dump cmd = [%02x, %02x, %02x, %02x]\n", __func__,
...
static int elan_ktf2k_ts_set_mode_state(struct i2c_client *client, int mode)
{
	...
	pr_info("[ELAN] %s: dump cmd: %02x, %02x, %02x, %02x\n", __func__,
		cmd[0], cmd[1], cmd[2], cmd[3]);
...
static int elan_ktf2k_ts_get_mode_state(struct i2c_client *client)
{
	...
	mode_state = buf[2];
	pr_info("[ELAN] %s: dump repsponse: %0x\n", __func__, mode_state);
				   ...
				   int bytes_to_recv)
{
	...
	if (rc != 8) {
		pr_info("[ELAN] %s: Read the first package error\n", __func__);
		mdelay(30);
		return -1;
	}
...
static void elan_ktf2k_ts_report_data(struct i2c_client *client, uint8_t *buf)
{
	...
	case 0x55: // chip may reset due to watch dog
		if (chip_type == 0x22) {
			if (buf[0] == 0x55 && buf[1] == 0x55 &&
				...
				pr_info("[ELAN] %s: get tp chip int gpio "
					"status: %d\n",
					__func__,
		...
		pr_info("[ELAN] %s: tp chip reset event may happen\n",
			__func__);
		break;
	case 0x66:
		if (buf[0] == 0x66 && buf[1] == 0x66 && buf[2] == 0x66 &&
		    buf[3] == 0x66)
			dev_dbg(&client->dev, "[ELAN] %s: calibration packet\n",
				__func__);
		else
			dev_dbg(&client->dev, "[ELAN] %s: unknow packet type\n",
				__func__);
		break;
	...
	case TEN_FINGERS_PKT:
		...
		if (num == 0) {
			...
			dev_dbg(&client->dev, "[ELAN] %s: %d fingers\n",
				__func__, num);
			input_report_key(idev, BTN_TOUCH, 1);
...
static int elan_ktf2k_ts_power_on(struct elan_ktf2k_ts_data *data, bool on)
{
	...
	if (rc < 0) {
		dev_err(
		    &data->client->dev,
		    "[ELAN] %s: Regulator vcc_ana set_opt failed rc = [%d]\n",
		    __func__, rc);
		return rc;
	}
	...
	if (rc) {
		dev_err(
		    &data->client->dev,
		    "[ELAN] %s: Regulator vcc_ana enable failed rc = [%d]\n",
		    __func__, rc);
	...
	if (data->pdata->i2c_pull_up) {
		...
		if (rc < 0) {
			dev_err(&data->client->dev, "[ELAN] %s: Regulator "
						    "vcc_i2c set_opt failed rc "
						    "= [%d]\n",
				__func__, rc);
		...
		if (rc) {
			dev_err(&data->client->dev, "[ELAN] %s: Regulator "
						    "vcc_i2c enable failed rc "
						    "= [%d]\n",
				__func__, rc);
			goto error_reg_en_vcc_i2c;
		}
	} else {
		pr_info("[ELAN] %s: We do not have i2c_pull_up\n", __func__);
	}
					     ...
					     bool on)
{
	...
	if (IS_ERR(data->vcc_ana)) {
		...
		dev_err(&data->client->dev,
			"[ELAN] %s: Regulator get failed vcc_ana rc = [%d]\n",
			__func__, rc);
		return rc;
	}
	...
	if (regulator_count_voltages(data->vcc_ana) > 0) {
		...
		if (rc) {
			dev_err(
			    &data->client->dev,
			    "[ELAN] %s: Regulator set_vtg failed rc = [%d]\n",
			    __func__, rc);
	...
	if (data->pdata->i2c_pull_up) {
		...
		if (IS_ERR(data->vcc_i2c)) {
			...
			dev_err(&data->client->dev,
				"[ELAN] %s: Regulator get failed rc = [%d]\n",
				__func__, rc);
			goto error_set_vtg_vcc_ana;
		}
		if (regulator_count_voltages(data->vcc_i2c) > 0) {
			...
			if (rc) {
				dev_err(&data->client->dev,
					"[ELAN] %s: Regulator set_vtg failed "
					"rc = [%d]\n",
					__func__, rc);
			       ...
			       const struct i2c_device_id *id)
{
	...
	pr_info("[ELAN] %s: client->addr = [0x%x]  name = [%s]\n", __func__,
		client->addr, client->name);

	if (client->dev.of_node) {
		pr_info("[ELAN] %s: of_node\n", __func__);
		pdata = devm_kzalloc(
		    &client->dev, sizeof(struct elan_ktf2k_i2c_platform_data),
		    GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev,
				"[ELAN] %s: Failed to allocate memory\n",
				__func__);
			return -ENOMEM;
		}
	...
	pr_info(
	    "[ELAN] %s: pdata->intr_gpio = [%d], pdata->reset_gpio = [%d]\n",
	...
	if (gpio_is_valid(pdata->hw_det_gpio)) {
		...
		pr_info("[ELAN] %s: touch_panel_type = [%d]\n", __func__,
			touch_panel_type);
	} else {
		pr_info("[ELAN] %s: ELAN TOUCH HW DET GPIO IS WRONG\n",
			__func__);
	}
	...
	if (fw_err == -136) {
		pr_err("[ELAN] %s: No Elan chip inside, fw_err = [%d]\n",
		       __func__, fw_err);
		err = -ENODEV;
		goto err_create_wq_failed;
	} else if (fw_err < 0) {
	...
	dev_info(&client->dev,
		 "[ELAN] %s: Start touchscreen %s in interrupt mode\n",
		 __func__, ts->input_dev->name);
