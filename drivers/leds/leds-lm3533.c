                                  ...
                                  const char *buf, size_t count)
{
    ...
    i2c_smbus_write_byte_data(lm3533_client, LM3533_BRIGHTNESS_REGISTER_F, 0xFF);
                                    ...
                                    const char *buf, size_t count)
{
    ...
    if (button2 == 1) {
        pr_info("[%s] on %d\n", __func__, theme_color);
        button2_data[0] = brightness_table[(theme_color >> 16) & 255];
        ...
        pr_info("[%s] off\n", __func__);
        button2_data[0] = brightness_table[(0x00) & 255];
                                           ...
                                           const char *buf, size_t count)
{
    ...
    if (log_flag_setting)
        pr_info("sns_data[0] = %d, sns_data[1] = %d, sns_data[2]=%d\n", sns_data[0], sns_data[1], sns_data[2]);
                                       ...
                                       const char *buf, size_t count)
{
    ...
    pr_info("[%s] Enter LED Debug Mode\n", __func__);
...
static int lm3533_led_set(struct lm3533_led_data *led, unsigned long brightness)
{
    ...
    switch (id) {
    case LM3533_LED0:
        BANK_ENABLE = BANK_ENABLE & 35;
        rgb_enable = 100 * ((brightness >> 16) & 255 ? 1 : 0) + 10 * ((brightness >> 8) & 255 ? 1 : 0) + ((brightness) & 255 ? 1 : 0);
    ...
    case LM3533_LED1: // LVLED 4 BANK F
        ...
        if (brightness) {
            ...
            if (err)
                pr_err("notification on error\n");
        } else {
            BANK_ENABLE = BANK_ENABLE & 31; //011111
            if (err)
                pr_err("notification off error\n");
        }
        break;
    case LM3533_LED2: // HVLED
        if (led->lm3533_led_brightness) {
            ...
            err = i2c_smbus_write_byte_data(led->client, LM3533_BRIGHTNESS_REGISTER_A, brightness_table[led->lm3533_led_brightness]);
            if (err)
                pr_err("backlight set error %d", err);
        } else {
            /*by major, ignore the first one*/
            if (1 == ignoreFirstBacklightOff_event) {
                ignoreFirstBacklightOff_event = 0;
                pr_info("backlight off 1st ignored\n");
                break;
            }
            brightness_temp = 0;
    ...
    if (BANK_ENABLE != BANK_ENABLE_ORIGINAL) {
        ...
        msleep(5);
        if (err)
            pr_err("bank set error\n");

        if (log_flag_setting)
            pr_info("BANK_ENABLE = 0x%x\n", BANK_ENABLE);
    }
                            ...
                            struct lm3533_platform_data *pdata)
{
    ...
    if (err)
        pr_info("LM3533_CONTROL_BANK_A_PWM_CONFIGURATION\n");
    ...
    if (err)
        pr_info("LM3533_CONTROL_BANK_C_FULL_SCALE_CURRENT\n");
    ...
    err = i2c_smbus_write_i2c_block_data(lm3533_client, LM3533_CONTROL_BANK_A_B_BRIGHTNESS_CONFIGURATION, 5, LM3533_data);
    if (err)
        pr_info("LM3533_CONTROL_BANK_A_B_BRIGHTNESS_CONFIGURATION\n");
    ...
    if (err)
        pr_info("LM3533_OVP_FREQUENCY_PWM_POLARITY\n");
    ...
    err = i2c_smbus_read_byte_data(lm3533_client, LM3533_CURRENT_SINK_OUTPUT_CONFIGURATION1);
    pr_info("0x%x", err);
    ...
    for (i = 0; i < pdata->leds_size; i++) {
        ...
        if (err < 0) {
            dev_err(&client->dev, "couldn't set STATUS %s\n", led->ldev.name);
            goto exit;
        }
    }
    return 0;
                        ...
                        const struct i2c_device_id *id)
{
    ...
    pr_info("[%s] 0x%x", __func__, i2c_smbus_read_byte_data(client, LM3533_CURRENT_SINK_OUTPUT_CONFIGURATION1));

    if (lm3533_pdata == NULL) {
        dev_err(&client->dev, "lm3533_probe no platform data\n");
        pr_err("lm3533_probe no platform data\n");
        return -EINVAL;
    }
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
        ...
        pr_err("lm3533_probe insufficient functionality\n");
        return -ENODEV;
    }
    ...
    if (i2c_smbus_read_byte_data(client, LM3533_CURRENT_SINK_OUTPUT_CONFIGURATION1) != 0x92) {
        ...
        if (i2c_smbus_read_byte_data(client, LM3533_CURRENT_SINK_OUTPUT_CONFIGURATION1) != 0x92)
            pr_info("LM3533 initial value error\n");
    }
